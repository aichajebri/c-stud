...
//iterator pour parcourir tous les éléments de la liste et les affiche le
...
for(it=Notes.begin();it<Notes.end();it++){
    if(((*it).getMatid()==mat.getIdMat())&&((*it).getEtu().getNum()==etd.getNum())){
        nombreNote++;
...
void GestionClasses::read_Etudiants(){
    ifstream file("./db/Etudiants.txt");
    ...
    else cout << "Erreur dans l'ouvertur de Etudiants.txt";
...
void GestionClasses::read_Groupes(){
        ...
        while (file1 >> IdGR >> Num)
    {
        ...
        if( index1!=-1 &&index!=-1) this->Groupes[index1].Add_To_Tab_Etd(this->Etudiants[index]);
    }
    }
    else cout << "erreur dans l'ouverture du fichier ListeEtd! " << endl;
     file1.close();
    ...
    while (file2 >> IdGR >> IdGM)
    {
        ...
        index1=Verifier_Exist_Groupe(this->Groupes,IdGR);
        if(index1!=-1 && index!=-1)this->Groupes[index1].Add_To_Tab_Mod(this->GroupeModules[index]);
    }
    file2.close();
}
void GestionClasses::read_Groupes_Module(){
    ...
    while (file1 >> IdGM >> Idmat)
    {
        ...
        if(index1!=-1 &&index!=-1) this->GroupeModules[index1].add_To_Table_Mat(this->Matieres[index]);
    }
    file.close();
    file1.close();
}
void GestionClasses::read_Matieres(){
    ...
    while (file >>Cnss >> IdMat >> NomMat >> Coef)
    {
        ...
        if(trouve==1){ofstream f("logs_mat.txt",ios::app);f<<"un des éléments a été supprimer , cette matierere ne sera pas chargé. enseignant::"<<Cnss<<endl;}
...
void GestionClasses::read_notes(){
    ...
    {
        index1=Verifier_Exist_Etudiant(Etudiants,Num);
        index2=Verifier_Exist_Matiere(Matieres,IdMat);
        ...
        else{ofstream f("logs.txt", ios::app);f<<"un des éléments a été supprimer , cette note ne sera pas chargée::"<<note.getNote()<<"::"<<note.getType()<<endl;}
    }
    }
    else cout << "erreur dans l'ouverture du fichier Notes.txt! " << endl;
...
void GestionClasses::write_notes(){
    ...
    int n=Notes.size();
    for(int i=0;i<n;i++){
    Etudiant e=Notes[i].getEtu();
    Matiere m=Notes[i].getMat();
    file << e.getNum()<< " " << m.getIdMat()<<" "<<Notes[i].getType()<<" "<<Notes[i].getNote()<<endl ;
    }
    file.close();
}
